...
DEFAULT_ENDORSEMENT_FILE = "MAYA ENDORSEMENT 03262026.xlsx"
...
def calculate_agency_input_table_121_150(
    ...
    # Part 1: Endorsed handled = ACTIVE filtered by RECEIVED DATE + POUT filtered by RECEIVED DATE,
    # then combine and dedupe by ACCOUNT_KEY.
    ...
    # Part 4: Accounts pulled out = POUT only, filtered by placement/date, then count rows.
    pulled_out_filtered = pout_filtered
    pulled_out_count = pulled_out_filtered.height

    # Part 2: OSB Endorsed = same combined endorsed flow, deduped by account, then sum OB.
    ...
    # Part 3: Collected amount = PAYMENTS + SELECTIVES for 121-150 only in selected month/year.
    ...
    st.caption(
        ...
        f"{payments_sum or 0.0:,.2f} | SELECTIVES 121-150: {selectives_sum or 0.0:,.2f}"
    )
...
def calculate_agency_input_table_181_above(
    ...
    # For 181 endorsed handled, POUT is filtered by pulled out date in the previous month.
...
def run_ui():
    ...
    with st.expander("Options", expanded=False):
        use_local_folder = st.checkbox("Use local options folder file", value=False)
        st.caption("Local scan source: AUTOMATION SYSTEM/options (fallback: AUTOMATION SYSTEM root)")
        use_server_file = st.checkbox("Use server file by name", value=False)
        server_file_name = ""
        if use_server_file:
    ...
    if st.button("Submit", type="secondary", use_container_width=True):
        ...
        try:
            ...
            if selected_file is None and use_local_folder:
                local_files = get_local_endorsement_files()
                if local_files:
                    selected_file = local_files[0]
                    st.caption(f"Using local file: {selected_file.name}")
                else:
                    st.warning("No local endorsement file found. Falling back to uploaded file if available.")

            if selected_file is None:
                set_progress(100, "Stopped")
                st.error("No endorsement file selected. Enable local mode with a valid folder or upload a file.")
                return
