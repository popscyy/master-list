...
# ==============================================================================
# UTILITY FUNCTIONS (shared across all steps)
# ==============================================================================
...
# ==============================================================================
# STEP 1 — COMBINE THREE DAILY REMARK FILES
# ==============================================================================

def _map_to_base(df: pd.DataFrame) -> dict[str, str]:
    """Return {base_col: actual_col} using exact then case-insensitive match only."""
    actual = {col.strip(): col for col in df.columns}
    actual_lower = {col.strip().lower(): col for col in df.columns}
    result = {}
    for base in STEP1_BASE_COLUMNS:
        if base in actual:
            result[base] = actual[base]
        elif base.lower() in actual_lower:
            result[base] = actual_lower[base.lower()]
    return result
...
def normalize_step1_amount_text(df: pd.DataFrame) -> pd.DataFrame:
	...
	return cleaned
def format_step1_workbook_bytes(excel_bytes: bytes) -> bytes:
	"""[STEP 1] Force Date column B to yyyy-mm-dd and Account No column E to whole-number display."""
	buffer = BytesIO(excel_bytes)
	workbook = load_workbook(buffer)
	worksheet = workbook.active
	target_columns = [1, 22, 25, 28, 35, 36, 38, 39, 44]  # A, V, Y, AB, AI, AJ, AL, AM, AR
		...
		for row_idx in range(2, worksheet.max_row + 1):
			...
			if isinstance(value, datetime):
				...
				if not pd.isna(parsed_dt):
					parsed = parsed_dt.date() if hasattr(parsed_dt, "date") else parsed_dt
...
# ==============================================================================
# STEP 2 — BUILD DAILY REMARK & CALL LOGS
# ==============================================================================
...
@lru_cache(maxsize=1)
def load_disposition_reference() -> pl.DataFrame:
	"""[STEP 2] Cached load of disposition mapping file to avoid repeated disk reads."""
	return pl.read_csv("./resources/maya_dispositions.csv")
...
def polars_to_excel_bytes(
	...
	header_left_non_bold: bool = False,
	enforce_autofit: bool = False,
) -> bytes:
	...
	if left_align_col_indices or header_left_non_bold or enforce_autofit:
		buffer.seek(0)
		workbook = load_workbook(buffer)
		worksheet = workbook.active
		...
		if date_col_idx <= ws.max_column:
			for row_idx in range(2, ws.max_row + 1):
				...
				if isinstance(raw_value, datetime):
					...
					for date_fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%d", "%d/%m/%Y", "%m/%d/%Y", "%d/%m/%Y %H:%M:%S", "%m/%d/%Y %H:%M:%S", "%m/%d/%Y %I:%M:%S %p"):
						try:
							parsed = datetime.strptime(text_value, date_fmt)
							break
						except Exception:
							continue
					if pd.isna(parsed):
						# Fallback parse without dayfirst to avoid repeated warning spam for ISO timestamps.
...
def format_step3_workbook_bytes(excel_bytes: bytes) -> bytes:
	"""[STEP 3] Force Account No column F to whole-number display and autofit the sheet."""
	buffer = BytesIO(excel_bytes)
	workbook = load_workbook(buffer)
	worksheet = workbook.active
...
# ============================================================================================================================================================
# STEP 4 — MERGE ACCOUNTS (ENDORSEMENT FILE)
# ============================================================================================================================================================
...
def resolve_server_endorsement_file(server_input: str) -> tuple[Path | None, str | None]:
	...
	if len(matches) > 1:
		...
		return None, (
			...
			"Please paste a relative path, for example 'MONTH 2026\\file.xlsx'.\n"
			f"Matches:\n{preview}"
		)
...
# ============================================================================================================================================================
# FILL MISSING VALUES (POST-STEP 5 ENRICHMENT)
# ============================================================================================================================================================
...
def fill_missing_values_pipeline(
	...
	endorsement_source,
	workbook_password: str | None = None,
	preserve_feedback_workbook: bool = True,
	...
	feedback_output_columns = [col for col in feedback_df.columns if col in export_df.columns]
	export_df = export_df.loc[:, feedback_output_columns]
	output_name = f"{Path(feedback_file.name).stem}.xlsx"
	if Path(feedback_file.name).suffix.lower() == ".csv" or not preserve_feedback_workbook:
		output_bytes = to_output_excel(export_df)
		formatted_workbook = load_workbook(BytesIO(output_bytes))
		worksheet = formatted_workbook.active
		for header_cell in worksheet[1]:
			header_cell.font = Font(bold=False)
			header_cell.alignment = Alignment(horizontal="left", vertical="center")
		for col_idx in range(1, worksheet.max_column + 1):
			...
			worksheet.column_dimensions[get_column_letter(col_idx)].width = min(max_len + 2, 60)
		formatted_output = BytesIO()
		formatted_workbook.save(formatted_output)
		formatted_output.seek(0)
		output_bytes = formatted_output.getvalue()
	else:
...
def run_ui():
	...
	st.title("LOXON Remarks Report")
	st.caption("Single streamlined workflow.")
	...
	with center:
		...
		endo_masterfile_name = st.text_input(
			...
			help=(
				f"Fixed Server: {DEFAULT_SERVER_MASTERFILE_DIR}\n\n"
				"Example: MAYA ENDORSEMENT 04152026.xlsx or MARCH 2026\\MAYA ENDORSEMENT 04152026.xlsx"
			),
			key="single_endo_masterfile_name",
			disabled=disabled_inputs,
		)
		optional_endo_upload = st.file_uploader(
			"Optional: Upload Endorsement File",
			type=["xlsx"],
		...
		workbook_password = st.text_input(
			"Endorsment File Workbook Password",
		...
		fast_mode = st.checkbox(
			...
			value=True,
			help="Speeds up processing by skipping heavy workbook-style formatting while preserving output data.",
			key="single_fast_mode",
			disabled=disabled_inputs,
		)
	...
	if start_process:
		...
		try:
			...
			if fast_mode:
				st.session_state.step1_combined_daily_remark_bytes = format_step1_workbook_bytes(
					to_output_excel_raw(combined_df, sheet_name="sheet")
				)
			else: 
				st.session_state.step1_combined_daily_remark_bytes = format_step1_workbook_bytes(
					to_output_excel_preserve(
						combined_df,
						sheet_name="sheet",
					)
				)
			...
			dispo_list = load_disposition_reference()
			...
			st.session_state.step2_daily_remark_bytes = polars_to_excel_bytes(
				daily_remark,
				daily_remark_formats,
				left_align_col_indices=None if fast_mode else [5, 9],
			)
			...
			if (not fast_mode) and Path(step3_source_file.name).suffix.lower() in {".xlsx", ".xlsm", ".xltx", ".xltm"}:
				st.session_state.step3_call_type_bytes = format_step3_workbook_bytes(
					add_step3_call_type_preserve_workbook(step3_source_file, workbook_password=None)
				)
			else:
				st.session_state.step3_call_type_bytes = format_step3_workbook_bytes(
					to_output_excel_raw(step3_output_df, sheet_name="Call Logs")
				)
			...
			# Convert account_number to string to prevent Excel scientific notation (E+11) display
			if "account_number" in loxon_upload.columns:
				loxon_upload = loxon_upload.with_columns(
					pl.col("account_number").cast(pl.Utf8).alias("account_number")
				)
		
			st.session_state.step5_loxon_upload_bytes = polars_to_excel_bytes(
				loxon_upload,
				loxon_upload_formats,
				header_left_non_bold=True,
				enforce_autofit=True,
			)
			...
			final_bytes, final_unresolved_df, final_stats, final_name = fill_missing_values_pipeline(
				...
				preserve_feedback_workbook=not fast_mode,
			)
	...
	if st.session_state.final_output_bytes is not None:
		...
		try:
			...
			if "final_output_df" in st.session_state:
				final_output_df = st.session_state.final_output_df
			else:
				final_output_df = pd.read_excel(BytesIO(st.session_state.final_output_bytes))
			final_csv = final_output_df.to_csv(index=False, encoding="utf-8-sig")
			st.download_button(
				"Download Final Output (.csv)",
				data=final_csv,
				file_name=st.session_state.final_output_name.replace(".xlsx", ".csv"),
				mime="text/csv",
				use_container_width=True,
			)
		except Exception as e:
			st.info(f"CSV download unavailable: {e}")

	import_targets: list[tuple[str, bytes, str, Path]] = []
	if st.session_state.step1_combined_daily_remark_bytes and st.session_state.step1_combined_daily_remark_name:
		import_targets.append(
			(
				"Combined Daily Remark",
				st.session_state.step1_combined_daily_remark_bytes,
				st.session_state.step1_combined_daily_remark_name,
				resolve_step1_drr_target_dir(st.session_state.step1_combined_daily_remark_name),
			)
		)
	if st.session_state.step2_daily_remark_bytes and st.session_state.step2_daily_remark_name:
		import_targets.append(
			(
				"Extracted Daily Remark",
				st.session_state.step2_daily_remark_bytes,
				st.session_state.step2_daily_remark_name,
				Path(DEFAULT_SERVER_REMARK_REPORT_DIR),
			)
		)
	if st.session_state.step3_call_type_bytes and st.session_state.step3_call_type_name:
		import_targets.append(
			(
				"Call Type Mapping",
				st.session_state.step3_call_type_bytes,
				st.session_state.step3_call_type_name,
				Path(DEFAULT_SERVER_CALL_LOGS_DIR),
			)
		)
	if st.session_state.step4_merged_accounts_bytes and st.session_state.step4_merged_accounts_name:
		import_targets.append(
			(
				"Merged Accounts",
				st.session_state.step4_merged_accounts_bytes,
				st.session_state.step4_merged_accounts_name,
				resolve_step4_merged_accounts_target_dir(st.session_state.step4_merged_accounts_name),
			)
		)
	if st.session_state.final_output_bytes and st.session_state.final_output_name:
		import_targets.append(
			(
				"Final Output",
				st.session_state.final_output_bytes,
				st.session_state.final_output_name,
				resolve_step6_loxon_upload_target_dir(st.session_state.final_output_name),
			)
		)

	if import_targets:
		help_lines = [f"{label} -> {target_dir}" for label, _data, _name, target_dir in import_targets]
		help_text = "Server targets:\n" + "\n".join(help_lines)

		if st.button(
			"Import All Outputs to Respective Folders",
			type="primary",
			use_container_width=True,
			key="import_all_outputs_to_server",
			help=help_text,
		):
			imported_rows: list[str] = []
			failed_rows: list[str] = []
			for label, output_bytes, output_name, target_dir in import_targets:
				try:
					target_file = upload_output_to_server(output_bytes, output_name, target_dir)
					imported_rows.append(f"{label}: {target_file}")
				except Exception as exc:
					failed_rows.append(f"{label}: {exc}")

			if imported_rows:
				st.success("Imported files:\n" + "\n".join(imported_rows))
			if failed_rows:
	...
	if st.session_state.final_unresolved_df is not None and not st.session_state.final_unresolved_df.empty:
		st.warning("⚠️ Some rows in the **Feedback File** (Step 5 output) could not be fully filled.")
		st.markdown("""
**Troubleshooting Guide:**
- **Missing account_number in feedback** → Check the Feedback File for blank account numbers
- **No matching POUT record found** → Account exists in Feedback but not in Endorsement File POUT sheet
- **POUT match found but source values were blank** → Account matched but Endorsement File has no CPM ID / Account ID values to copy

**Which files to check:**
- Feedback File: `{final_output_name}` (Step 5 output)
- Endorsement File: POUT sheet (check account numbers and CPM_ID/ACCOUNT_ID columns)
		""".format(final_output_name=st.session_state.final_output_name or "MADRID_Feedback_*.xlsx"))
